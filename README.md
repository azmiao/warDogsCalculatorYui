# warDogsCalculatorYui

适用于 [YuiChyanBot](https://github.com/azmiao/YuiChyanBot) 的 WARDOGS(战狗) 火炮射击诸元计算 QQ机器人插件

- 在群聊中通过对话输入炮位与目标坐标，即可算出**距离、方位角、MIL 仰角、ΔX/ΔY**，
- 结果卡片图片的形式返回，支持 **L81 迫击炮**与 **SPH-2 自行火炮**
- 计算逻辑参考自开源项目 [apollyon-sys/wardogs-calculator](https://github.com/apollyon-sys/wardogs-calculator)

## 效果预览

![preview.png](preview.png)

## 使用

| 命令 | 说明 |
|:-----|:-----|
| `火力计算 105 115 110 120` | 一条命令算出射击诸元（炮位X 炮位Y 目标X 目标Y），默认 L81 迫击炮 |
| `火力计算 榴弹炮 100 100 115 120` | 指定武器计算，武器名放在坐标前 |
| `火力计算 x97.43, y109.27 x100.5, y120.3` | 支持带 x/y 标签的坐标 |
| `火力计算 锁定 105 115` | 锁定自己的炮位坐标 |
| `火力计算 110 120` | 已锁定炮位后，只发目标坐标即可计算 |
| `火力计算 解锁` | 解除炮位锁定 |
| `火力计算帮助` | 查看帮助图片 |

武器别名（省略时默认 L81 迫击炮）：

- L81 迫击炮：`迫击炮` / `l81` / `mortar`
- SPH-2 自行火炮：`榴弹炮` / `自行火炮` / `sph2` / `spg`

### 炮位坐标锁定

锁定自己的炮位坐标后，后续计算无需重复输入炮位，只需发送目标坐标即可，适合持续蹲守同一位置时使用：

| 操作 | 命令 | 说明 |
|:-----|:-----|:-----|
| 锁定 | `火力计算 锁定 105 115` | 保存炮位坐标；再次发送即**替换**为新炮位 |
| 计算 | `火力计算 110 120` | 单个坐标点视为目标，自动套用锁定炮位 |
| 查看 | `火力计算 锁定` | 查看当前锁定的炮位坐标 |
| 解锁 | `火力计算 解锁` | 清除锁定，恢复需输入完整坐标 |

> ⚠️ 锁定跨群生效（同一用户通用）。**更换位置后请重新锁定或解锁**，避免仍按旧炮位计算。
> 锁定状态下，结果卡片会以醒目的 🔒 横幅与高亮炮位坐标提示当前锁定，防止忘记更新。

### 坐标容错解析

炮位坐标在前、目标坐标在后，支持多种不规范写法：

| 写法 | 示例 |
|:-----|:-----|
| 纯数字 | `105 115 110 120` |
| 带 x/y 标签 | `x97.43, y109.27 x100.5, y120.3` |
| 标签乱序 | `y109.27 x97.43` 也能正确配对 |
| 逗号 / 分号 / 换行分隔 | `97.43,109.27,100.5,120.3` |
| 全角标点 | `x97.43，y109.27 x100.5，y120.3` |
| 括号包裹 | `(97.43, 109.27) (100.5, 120.3)` |

> 推荐直接在游戏内标记坐标到聊天框里，会显示为`x97.43, y109.27`，然后可以直接快速复制给BOT进行计算

## 计算说明

- 坐标换算：1 游戏坐标 = 100 米。
- 距离：`hypot(ΔX, ΔY) × 100` 米。
- 方位角：正北为 0°，顺时针增大（90° 为正东）。
- 仰角：按距离对弹道表线性插值；L81 为单一解，SPH-2 在可用距离给出低角 / 高角双解。
- 射程判定：L81 为 132–684 m，SPH-2 为 780–2629 m，越界时标注"超出射程"。

## 目录结构

```
├── __init__.py           # Service 定义与触发器
├── ballistics.py         # 弹道表加载与 MIL 插值
├── calculator.py         # 坐标解析、距离/方位角/ΔXΔY 计算
├── store.py              # 炮位坐标锁定持久化（RocksDB）
├── render.py             # 结果卡片渲染（Playwright），含文本降级
├── weapons.json          # 武器与弹道数据
├── template/result.html  # 结果卡片模板
├── HELP.md               # 帮助文档（框架自动生成帮助图）
└── tests/                # 测试与数据生成脚本
```

## 使用方式

1. 将仓库 Git Clone 到 `YuiChyanBot\yuiChyan\plugins`插件目录下
2. 在 `yuiChyan/config/extra_plugins.json5` 中新增一条配置：

    ```json5
    {
        "warDogsCalculatorYui": "战狗火力计算器",
    }
    ```

3. 重启BOT即可生效

## 许可

- 本项目代码以 MIT 许可发布，见 [LICENSE](LICENSE)。
- WARDOGS 相关的游戏数据与素材版权归其各自所有者所有。
